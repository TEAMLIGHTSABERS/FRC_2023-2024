...
## Check That "{username}Dev" is Rebased
After you have tested your development code and you feel that the code is good enough for competition, the next step is to check for any changes that have been made on the "main" branch while you were devloping your code.  To do this, make a "pull" request in GitHub Web.  The request is to pull the changes from the "main" branch into your "{username}Dev" Branch.  If there are no changes identified in this pull request then "base" code for "{username}Dev" still matches the "main" branch code.  In other words, no other developer has delivered new changes to the Robot competition code while "{username}Dev" branch code was being implememented and tested.  The "{username}Dev" branch is called "rebased".  Skip to [Deliver Code to Main Branch](##Deliver_Code_to_Main_Branch).

If the "pull" request from the "main" branch to "{username}Dev" branch shows that changes have been made, then another developer has delivered changes to the competition code since "{username}Dev started implementing and testing. To prevent overwriting delivered changes to the "main" branch, all of these changes must be accepted into the {username}Dev branch.  When these changes are accepted, it is possible that they will conflict with, or break, the code that is currently in the "{username}Dev" branch.  Check that the code in "{username}Dev" still builds and test it before repeating the this process.  Repeat this process until the "{username}Dev" branch is rebased, as described above.

## Deliver Code to Main Branch
After the "{username}Dev" branch is rebased, a "pull" request to pull the changes from "{username}Dev" branch into the "main" branch can be made and there should be no conflict.
